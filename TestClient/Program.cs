...
namespace TestClient
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine("1、登录【admin】 2、登录【system】 3、登录【错误用户名密码】 4、查询DemoAAPI数据  5、查询DemoBAPI数据 ");
        ...
        static dynamic NullLogin()
        {
            ...
            loginRequest.AddJsonBody(new { username = "gswaa", password = "111111" });
            //或用用户名密码查询对应角色
        ...
        static dynamic SystemLogin()
        {
            var loginClient = new RestClient(_url);
            var loginRequest = new RestRequest("/authapi/login", Method.POST);
            loginRequest.AddJsonBody(new { username = "paul", password = "111111" });
        ...
        static dynamic AdminLogin()
        {
            var loginClient = new RestClient(_url);
            var loginRequest = new RestRequest("/authapi/login", Method.POST);
            loginRequest.AddJsonBody(new { username = "ryan", password = "12345678" });
